  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0A1628' }}>
        <div className="w-12 h-12 border-4 border-[#BE123C] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-32">
        <div className="text-center">
          ...
          <Link href="/careers" className="inline-flex items-center gap-2 px-7 py-3.5 bg-[#BE123C] text-white font-semibold rounded-xl hover:bg-[#9F1239] transition-all duration-200">View All Jobs</Link>
        </div>
      </div>
    );
  }
                ...
                <div className="flex flex-wrap gap-4 p-6 rounded-2xl border border-gray-100"
                  ...
                  {[
                    ...
                    <div key={item.label} className="flex items-center gap-2 text-gray-600 text-sm">
                      <span className="text-[#BE123C]">{item.icon}</span>
                      {item.label}
                    </div>
                  ))}
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(job.primarySkills)
                      ...
                      <span key={s.trim()} className="px-3 py-1.5 rounded-full text-sm font-medium"
                        style={{ background: 'rgba(212,23,74,0.22)', color: '#BE123C' }}>
                        {s.trim()}
                      </span>
                    ))}
                  </div>
                  ...
                  <ul className="space-y-2">
                    {job.responsibilities.split('\n').filter(Boolean).map((r, i) => (
                      <li key={i} className="flex items-start gap-3 text-gray-600">
                        <div className="w-2 h-2 rounded-full bg-[#BE123C] mt-2 flex-shrink-0" />
                        {r}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="sticky top-28 p-7 rounded-2xl border border-gray-100"
                  ...
                  {submitted ? (
                    <div className="text-center py-8">
                      ...
                      <Link href="/careers" className="inline-flex items-center justify-center gap-2 w-full px-7 py-3.5 bg-[#BE123C] text-white font-semibold rounded-xl hover:bg-[#9F1239] transition-all duration-200">
                        View More Jobs
                      </Link>
                    </div>
                  ) : (
                      ...
                      <form onSubmit={handleSubmit} className="space-y-4">
                        {[
                          ...
                          <div key={field.name}>
                            ...
                            <input
                              ...
                              className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#BE123C] transition-colors"
                            />
                          </div>
                        ))}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-[#BE123C] hover:file:bg-blue-100"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#BE123C] resize-none transition-colors"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#BE123C]"
                          />
                        </div>
                        ...
                        <button type="submit" className="inline-flex items-center justify-center gap-2 w-full px-7 py-3.5 bg-[#BE123C] text-white font-semibold rounded-xl hover:bg-[#9F1239] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
