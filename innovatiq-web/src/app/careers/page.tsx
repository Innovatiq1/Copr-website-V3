    ...
    <>
      ...
      <section className="relative py-20 overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] pointer-events-none"
          style={{ background: 'radial-gradient(circle at bottom left, rgba(244,63,94,0.03) 0%, transparent 60%)' }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 items-stretch">
            {[
              { icon: '🚀', title: 'Challenging Work', description: 'Work on complex, high-impact projects for enterprise clients across the region.', color: '#BE123C' },
              { icon: '📚', title: 'Continuous Learning', description: 'Generous training and certification budgets to keep your skills current.', color: '#F43F5E' },
              ...
              <AnimatedSection key={b.title} delay={i * 80} className="h-full">
                <div className="text-center p-7 hover:-translate-y-1 transition-all duration-300 h-full"
                  style={{
                    background: 'linear-gradient(#FFFFFF, #FFFFFF) padding-box, linear-gradient(to right, #BE123C 0%, #BE123C 20%, rgba(190,18,60,0.80) 45%, rgba(190,18,60,0.33) 70%, transparent 90%) border-box',
                    borderStyle: 'solid',
      ...
      <section className="relative py-20 overflow-hidden" style={{ background: 'linear-gradient(160deg, #FFFFFF 0%, #F9FAFE 100%)' }}>
        ...
        <div className="absolute inset-0 pointer-events-none select-none overflow-hidden">
          ...
          <div className="absolute top-0 right-0 w-[400px] h-[400px]"
            style={{ background: 'radial-gradient(ellipse at top right, rgba(244,63,94,0.04) 0%, transparent 60%)' }} />
          {/* Subtle dot grid */}
          <div className="absolute inset-0 opacity-[0.018]"
            style={{ backgroundImage: 'radial-gradient(circle, #BE123C 1px, transparent 1px)', backgroundSize: '36px 36px' }} />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {careers.map((job, i) => (
              <AnimatedSection key={job._id} delay={i * 60}>
                <div className="p-5 pl-6 hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row sm:items-center gap-5"
                  style={{
                    background: 'linear-gradient(#FFFFFF, #FFFFFF) padding-box, linear-gradient(to bottom, #BE123C 0%, #BE123C 20%, #BE123CCC 45%, #BE123C55 70%, transparent 90%) border-box',
                    borderStyle: 'solid',
                    borderColor: 'transparent',
                    borderTopWidth: '0',
                    borderLeftWidth: '4px',
                  ...
                  <div className="shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center self-start sm:self-center"
                    style={{ background: 'linear-gradient(135deg, rgba(190,18,60,0.12), rgba(190,18,60,0.05))', border: '1px solid rgba(190,18,60,0.18)' }}>
                    <Briefcase size={22} style={{ color: '#BE123C' }} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-slate-500 font-medium">
                      ...
                      {(job.experienceLevel || job.experience) && (
                        <span className="flex items-center gap-1.5"><Briefcase size={13} style={{ color: '#BE123C' }} />{job.experienceLevel || job.experience}</span>
                      )}
                      {job.employmentType && (
                        <span className="flex items-center gap-1.5"><Clock size={13} style={{ color: '#BE123C' }} />{job.employmentType}</span>
                      )}
                    </div>
                    {(() => {
          ...
          <AnimatedSection className="mt-10">
            <div className="relative overflow-hidden rounded-2xl flex flex-col sm:flex-row items-stretch"
              style={{
                background: 'linear-gradient(135deg, #FFF1F2 0%, #FFF9F5 60%, #FFFBF5 100%)',
                border: '1.5px solid rgba(190,18,60,0.18)',
                boxShadow: '0 4px 24px rgba(190,18,60,0.08)',
              }}>
              ...
              <div className="relative z-10 flex-1 px-8 py-12">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5"
                  style={{ background: 'rgba(190,18,60,0.09)', border: '1px solid rgba(190,18,60,0.18)' }}>
